...
// OutboxStore persists queued messages so work survives process restarts.
// Implementations must make Claim atomic so multiple workers cannot own the
// same message lease concurrently.
type OutboxStore interface {
	Enqueue(ctx context.Context, msg *Message) error
	Claim(ctx context.Context, workerID string, limit int, lease time.Duration) ([]*Message, error)
	Ack(ctx context.Context, messageID string) error
	Fail(ctx context.Context, messageID string, retryAt time.Time, reason string) error
	Migrate(ctx context.Context) error
}

// PostgresOutboxStore is a PostgreSQL-backed durable outbox.
//
...
WHERE id = $3`, workerID, lockedUntil, m.id); err != nil {
	...
	for _, m := range messages {
		...
		out = append(out, &Message{
			ID: m.id, IdempotencyKey: m.key, Topic: m.topic, Payload: payload,
			Timestamp: m.created, Retries: m.retries, MaxRetries: m.maxRetries,
		})
	}
	return out, nil
}
