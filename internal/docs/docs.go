...
func (g *DocGenerator) GenerateAPIDocs(endpoints []Endpoint) string {
	...
	fmt.Fprintf(&sb, "# %s API Documentation\n\n", g.projectName)
...
func (cg *ContributorGuide) Generate() string {
	var sb strings.Builder
	fmt.Fprintf(&sb, "# Contributing to %s\n\n", cg.projectName)
	fmt.Fprintf(&sb, "Thank you for your interest in contributing to **%s**!\n\n", cg.projectName)
	if cg.repoURL != "" {
...
func (cd *ConfigDoc) Generate() string {
	...
	for _, key := range cd.order {
		...
		fmt.Fprintf(&sb, "- `%s` maps to env var `%s`\n", cd.fields[key].Name, envName)
	}
	return sb.String()
}
