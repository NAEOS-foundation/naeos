...
### Added
- **Config providers** — `internal/configprovider`: `Provider` interface with `Scheme` type, env (`env:VAR`), file (`file:/path`), K8s secret (`secret:ns/name/key`), and Vault KV (`vault:path#key`) providers over an in-memory secret store, ordered `Chain` resolution, and `Resolver.ResolveMap` for whole config files; unknown prefixes pass through as plain values. CLI: `naeos config resolve <config.json> --secret --vault`, `naeos config test <reference>`, `naeos config sources`. 17 tests including concurrency.
- **Helm chart scaffolding** — `internal/helm`: `Chart` model (Chart.yaml metadata, values, templates, maintainers, keywords), `NewChart` with sensible defaults (image, service, ingress, HPA, resources, serviceAccount), template renderers for deployment/service/ingress/hpa/serviceaccount/_helpers/NOTES.txt, `Validate`, `WriteToDisk`/`LoadFromDisk` (chart structure on disk), YAML parse/flatten for values. CLI: `naeos helm init <name> --version --app-version --description` (create chart), `naeos helm chart render <dir>` (render chart files), `naeos helm chart validate <dir>` (validate). 22 tests covering creation, defaults, validation, rendering, YAML roundtrip, template presence.
- **Air-gap bundles** — `internal/airgap`: `Bundle` manifest (charts, images, SBOMs, signatures, manifest hash), `Builder` collecting chart archives (*.tgz), image list (name:tag), SBOM JSON, and signature bundles (*.sig.json), `WriteBundle`/`ReadBundle` tar.gz archive IO, `Extract` with hash verification and path traversal protection, `VerifyChecksum` tamper detection. CLI: `naeos airgap bundle --name X --version Y --charts-dir --images-file --sbom --signatures-dir`, `naeos airgap import <bundle.tar.gz>`, `naeos airgap inspect <bundle.tar.gz>`. 15 tests covering image parsing, checksum, build/extract roundtrip, invalid bundles, path safety.
- **Version bump** — 3.3.0 → 3.4.0.

## [3.3.0] - 2026-09-04

### Added
- **SBOM generation (CycloneDX)** — `internal/sbom` CycloneDX 1.5 model (`BOM`, `Component`, `Hash`, `Dependency`), `Generator` producing BOMs from component inventory or directory scan (`FromDir`), `Write`/`Load`/`Marshal`/`Unmarshal` for JSON persistence, `NewSerialNumber()` RFC 4122 v4 UUID, `Purl()` package URL builder. CLI: `naeos sbom generate --project X --version Y --dir /path` (file-level SBOM) and `naeos sbom generate --output bom.json` (write to file); `naeos sbom verify bom.json` (format/hash/structure checks) and `naeos sbom inspect bom.json` (dump). 20 tests covering BOM creation, serial numbers, purl, generate/empty, marshal/unmarshal roundtrip, invalid format/JSON, hash population, hash preservation, dependencies, component options, component count, encode.
- **SBOM verifier** — `internal/verification.SBOMVerifier` implementing the `Verifier` interface: verifies evidence record artifact hash matches SBOM content hash (live `ArtifactSource` or expected map). 7 tests: match/mismatch/no-hash/source-found/source-not-found/source-mismatch/name.
- **Artifact signing (Ed25519)** — `internal/signing`: `KeyPair` generation (`GenerateKeyPair`), Ed25519 sign/verify with SHA-256 digest, `Bundle` model (spec version, algorithm, artifact hash, signature, public key, signer, timestamp), `Write`/`Load`/`VerifyFile` for JSON persistence. `Hash`/`HashFile` SHA-256 content hashing. CLI: `naeos sign keygen` (generate key pair), `naeos sign sign <artifact> --key <keyfile>` (sign file, writes `<artifact>.sig.json`), `naeos sign verify <sig.json>` (verify signature). 24 tests covering key generation, base64 roundtrip, sign/verify, tampered hash/signature, wrong key, nil bundle, unsupported version, invalid encoding, sign options, JSON roundtrip, file verify/load, concurrent safety.
- **Version bump** — 3.2.0 → 3.3.0.

## [3.2.0] - 2026-09-03

### Added
- **Production server daemon** — `naeos serve` runs NAEOS as a production daemon (`internal/serve`): multi-listener over plain HTTP and TLS (TLSv1.3 minimum), graceful shutdown on SIGINT/SIGTERM with configurable timeout, YAML server config file (`naeos serve config` prints a starter), and systemd integration (`naeos serve install`/`uninstall` writes and removes a unit that launches `naeos serve run` with auto-restart, optional user, environment, and `LimitNOFILE`). `naeos serve` / `serve run` accept `--config`, `--port`, `--auth`, `--jwt-secret`, `--tls-cert`, `--tls-key`. API listeners reuse the existing REST API server with middleware/metrics; `healthz`/`readyz` ready for kube/systemd probes. 20+ tests covering config parsing/validation, multi-listener lifecycle, TLS, and systemd unit rendering.
- **Policy registry + control plane (governance)** — `internal/governance/policy` model + thread-safe `Registry` (register/get/set-active/delete/list/versioning, scoped rules, `gte`/`lte` evaluator operators) and `internal/governance/control` `ControlPlane` issuing deterministic `ALLOW`/`DENY`/`REQUIRE_APPROVAL` decisions (fail-closed by default, strictest-wins aggregation, context injection). New CLI: `naeos policy register|list|get|set-active|validate` and `naeos control evaluate` with JSON persistence under `~/.config/naeos/policies/`.
- **Runtime execution gateway** — `internal/runtime/gateway` ExecutionGateway: the single enforcement boundary between agent intent and tool execution. Integrates with the Control Plane to authorize every tool request before sandboxed execution. `AgentAdapter` interface for plugging in AI coding agents (Claude, Copilot, Cursor, etc.); `Sandbox` interface for isolated execution; `Restriction` model for policy-based command restrictions (wildcard tool/action/environment patterns, deny overrides policy). `DefaultSandbox` with blocked/allowed tool lists. CLI: `naeos runtime exec --tool X --action Y --environment Z --actor W` with JSON output. Full test coverage: allow/deny/approval-required, restriction matching, adapter integration, concurrent safety, duration tracking, deterministic decisions, context injection, and end-to-end integration with real policy registry.
