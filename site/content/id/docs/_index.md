---
title: Dokumentasi
description: Pelajari cara menggunakan NAEOS dengan panduan dan referensi yang komprehensif.
---

Selamat datang di dokumentasi NAEOS. Di sini Anda akan menemukan semua yang diperlukan untuk memulai dan menguasai platform.

## Memulai

- **[Panduan Awal]({{< relref "docs/getting-started.md" >}})** — Panduan memulai cepat untuk instalasi dan menjalankan pipeline pertama
- **[Instalasi](/id/docs/installation/)** — Petunjuk instalasi terperinci untuk semua platform
- **[Prinsip Inti](/id/docs/core-principles/)** — 8 prinsip engineering yang memandu NAEOS

## Arsitektur & Desain

- **[Arsitektur](/id/docs/architecture/)** — Penjelasan mendalam tentang arsitektur NAEOS
- **[Struktur Proyek](/id/docs/project-structure/)** — Layout repositori dan organisasi direktori
- **[Bahasa Spesifikasi](/id/docs/spec-language/)** — Sintaks dan fitur Bahasa Spesifikasi NAEOS v2
- **[Model NEIR](/id/docs/neir-model/)** — Representasi Antara Rekayasa NAEOS yang kanonikal
- **[Pipeline Engine](/id/docs/pipeline-engine/)** — Pipeline DAG 11-tahap dari parsing hingga output artifact

## Fitur

- **[Kompiler AI](/id/docs/ai-compiler/)** — Kompilasi NEIR ke set instruksi AI untuk 7 platform
- **[Context Bundles](/id/docs/context-bundles/)** — Hasilkan ringkasan proyek optimal untuk LLM
- **[Profil Industri](/id/docs/profiles/)** — Profil yang sudah dikonfigurasi untuk SaaS, AI, FinTech, Healthcare, Government
- **[Plugin SDK](/id/docs/plugin-sdk/)** — Perluas NAEOS dengan plugin dan generator kustom
- **[Validasi](/id/docs/validation/)** — Validasi multi-level untuk spesifikasi, modul, layanan, dan kebijakan
- **[Governance](/id/docs/governance/)** — Sistem kebijakan, alur kerja review, dan audit trail

## Referensi

- **[Referensi CLI](/id/docs/cli-reference/)** — Referensi perintah lengkap untuk 200+ perintah CLI
- **[Build Terdistribusi](/id/docs/distributed-builds/)** — Paralelkan eksekusi pipeline di banyak worker
- **[Dashboard](/id/docs/dashboard/)** — Pantau aktivitas pipeline dan kesehatan secara real time
- **[Glosarium](/id/docs/glossary/)** — Istilah-istilah penting dan definisinya
- **[Berkontribusi](/id/docs/contributing/)** — Panduan untuk berkontribusi ke NAEOS

## Sumber Daya Eksternal

- [Repositori GitHub](https://github.com/NAEOS-foundation/naeos) — Kode sumber dan isu
- **[Referensi API](/id/docs/api/)** — Dokumentasi API interaktif
- [OpenAPI Spec](https://github.com/NAEOS-foundation/naeos/blob/main/docs/openapi.yaml) — Spesifikasi OpenAPI 3.0