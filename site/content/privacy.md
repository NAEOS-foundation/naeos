---
title: Privacy Policy
description: How we handle your data and privacy.
---

## Information We Collect

NAEOS is a command-line tool that runs locally on your machine. We do not collect or transmit any data from your usage of the NAEOS CLI tool itself.

When you visit the NAEOS website (naeos.dev), we may collect:

- **Anonymous usage data** via privacy-focused analytics (no personal data)
- **GitHub API data** — when you visit pages that display GitHub stats (stars, forks), your browser fetches data from the public GitHub API. No authentication data is collected.

## Data We Do NOT Collect

- NAEOS does not collect telemetry from the CLI tool
- NAEOS does not track your specifications, projects, or generated code
- NAEOS does not use cookies for tracking purposes (only a local storage flag for cookie consent)

## Third-Party Services

- **GitHub API** — Used to display repository statistics. Subject to GitHub's privacy policy.
- **Google Fonts** — Used for typography. Subject to Google's privacy policy.

## Contact

For privacy-related questions, please open an issue on the [NAEOS GitHub repository](https://github.com/NAEOS-foundation/naeos).