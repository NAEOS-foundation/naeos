...
export function evaluateAuthorization(input: AuthorizationRequest): AuthorizationResult {
  ...
  if (!input.approval) {
    ...
    if (!input.approval.approvalId) reasons.push("approval-id-missing");
    if (!input.approval.approvedBy) reasons.push("approver-missing");
    if (!input.approval.approvedAt) reasons.push("approval-time-missing");
